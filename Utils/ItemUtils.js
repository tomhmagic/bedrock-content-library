export class ItemUtils{
    ...
    static compareItemStacks(itemStack, otherItemStack, compareAmount = false){
        ...
        //Item Other NBT
        if (itemStack.maxAmount !== otherItemStack.maxAmount) return false;
        ...
        if (itemStackLore.length !== otherItemStackLore.length) return false;
        if(itemStackLore.length > 0){
            for (let i = 0; i < itemStackLore.length; i++) {
                if (itemStackLore[i] !== otherItemStackLore[i]) return false;
            }
        }
        ...
        if(itemStackTags.length > 0){
            for (let i = 0; i < itemStackTags.length; i++) {
                if (itemStackTags[i] !== otherItemStackTags[i]) return false;
            }
        }
        ...
        if(itemStackComponents.length > 0){
        ...
        if(itemStackDPs.length > 0){
            for (let i = 0; i < itemStackDPs.length; i++) {
                if (itemStack.getDynamicProperty(itemStackDPs[i]) !== otherItemStack.getDynamicProperty(itemStackDPs[i])) return false;
            }
        }
